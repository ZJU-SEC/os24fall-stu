...
阅读文档中关于 [Makefile](#makefile) 的章节，以及工程文件中的 Makefile 文件，根据注释学会 Makefile 的使用规则后，补充 `lib/Makefile`，使工程得以编译。（Tips: 可参考其他文件夹的 Makefile）
...
1. 设置 `stvec`， 将 `_traps` ( `_traps` 在 4.3.2 中实现 ) 所表示的地址写入 `stvec`，这里我们采用 `Direct 模式`, 而 `_traps` 则是 trap 处理入口函数的基地址。（Tips: `la` 指令以及刚才写过的 CSR 读写指令）
2. 开启时钟中断，将 `sie[STIE]` 置 1。
3. 设置第一次时钟中断，参考 `clock_set_next_event()` ( `clock_set_next_event()` 在 4.3.4 中介绍 ) 中的逻辑用汇编实现。（Tips: 可通过 `rdtime` 指令获取时间）
